...
import { Issue, PresetCase, SeverityType, CategoryType } from "../types";
...
export default function PremiumDashboard({ onToggleDemo1 }: PremiumDashboardProps) {
  ...
  const [projectUrl, setProjectUrl] = useState("http://localhost:3000/staging/saas-hero-component");
  ...
  const [analyzeAllResolutions, setAnalyzeAllResolutions] = useState(true);
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div class="space-y-1">
                <div class="flex items-center gap-2 flex-wrap">
                  <span class="px-2 py-0.5 text-[9px] font-mono tracking-widest uppercase font-extrabold rounded ${
...
<body class="bg-slate-950 text-slate-100 min-h-screen pb-16">
  ...
  const runVisualQAAudit = async () => {
    ...
    if (figmaMode === "upload") {
      ...
      try {
        ...
        console.error("Figma URL pre-render failed, fallback to preset baseline template:", err);
      }

      if (!activeDesignImage) {
        // Fallback to active preset model so it never empty-fails
        activeDesignImage = (selectedScenarioId === "preset-hero" ? presetCatalog.designImage : presetPricing.designImage);
      }
    ...
    if (projectMode === "upload") {
      ...
      try {
        const response = await fetch("/api/capture", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: projectUrl, type: "staging" })
        });
        if (response.ok) {
          const resData = await response.json();
          if (resData.success && resData.base64Image) {
        ...
        console.error("Staging URL preview render failed:", err);
      }

      if (!activeSiteImage) {
        activeSiteImage = (selectedScenarioId === "preset-hero" ? presetCatalog.siteImage : presetPricing.siteImage);
      }
    }
  ...
  return (
    <>
      ...
      {/* FULLSCREEN VISUAL QA AUDIT PARAMETERS SECTION FIRST */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div className="bg-white border border-slate-200 p-6 sm:p-8 rounded-3xl shadow-xs space-y-8 animate-fade-in">
          ...
          <div className="border-b border-slate-100 pb-5">
            ...
            <h2 className="font-display font-black text-slate-800 text-xl tracking-tight mt-1">
              Visual QA Audit Parameters
            </h2>
            <p className="text-slate-450 text-xs mt-0.5">
              Set up your visual standards and compare screenshots across screen sizes, making it easy to catch layout discrepancies across desktop, tablet, and mobile views.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* STAGE A: FIGMA DESIGN INPUTS & LIVE CROP FRAME */}
            <div className="bg-slate-50/40 rounded-2xl border border-slate-150 p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100">
                <div className="flex items-center gap-2.5">
                  <div className="w-8.5 h-8.5 rounded-xl bg-pink-100 text-pink-600 flex items-center justify-center">
                    <Figma size={16} />
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-800 block leading-tight">Figma Design</span>
                  </div>
                </div>
            ...
            {/* STAGE B: PROJECT IMPLEMENTATION SNAPSHOTS & LIVE CROP FRAME */}
            <div className="bg-slate-50/40 rounded-2xl border border-slate-150 p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100">
                <div className="flex items-center gap-2.5">
                  <div className="w-8.5 h-8.5 rounded-xl bg-sky-100 text-sky-600 flex items-center justify-center">
                    <Globe size={16} />
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-800 block leading-tight">Developed Project</span>
                  </div>
                </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            ...
            <button
              type="button"
              onClick={runVisualQAAudit}
              disabled={isAuditing}
              className="w-full py-4 text-sm font-bold rounded-2xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white flex items-center justify-center gap-2 shadow-sm transition-all scale-100 hover:scale-[1.005] border-none cursor-pointer"
            >
              <Play size={13} fill="white" />
              Initiate Comparative Visual QA Audit ({getResolutionLabel()})
            </button>
          </div>
      ...
      {/* SECONDARY ROW: SIDE BY SIDE HISTORICAL ARCHIVE & GUIDELINE CLASSIFICATIONS */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          
          {/* LEFT 8 COLUMNS: PERSISTENT Comparative Visual QA Audit Logs */}
          <div className="lg:col-span-12">
            <div id="comparative-archives-card" className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-xl shadow-slate-200/45 space-y-6 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/55 border-t-4 border-t-emerald-600 relative overflow-hidden">
              <div className="flex items-center justify-between">
                <h3 className="font-display font-bold text-slate-800 text-lg tracking-tight">Recent Runs</h3>
                <span className="text-[10px] font-mono bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{historyRuns.filter(r => !r.archived).length} Active</span>
              </div>
              ...
              <div className="grid grid-cols-12 gap-4 text-[10px] font-mono uppercase tracking-wider text-slate-400 font-bold px-3">
                ...
                <div className="col-span-7">Bugs Status</div>
              </div>

              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                {historyRuns.filter(r => !r.archived).length === 0 ? (
                  <div className="text-center py-6 text-xs text-slate-400 border border-dashed rounded-xl">No active runs available</div>
                ) : (
                  historyRuns.filter(r => !r.archived).map((run: any) => (
                    <button
                      key={run.id}
                      onClick={() => loadHistoryItem(run)}
                      className="w-full grid grid-cols-12 gap-4 items-center p-3 rounded-xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/50 transition-all text-left"
                    >
                      ...
                      <div className="col-span-2 text-[10px] text-slate-600 font-mono">
                        {new Date(run.startedAt).toLocaleDateString()}
                      </div>
                      <div className="col-span-7 w-full max-h-16 overflow-y-auto text-[10px] text-slate-600">
                        {run.issues && run.issues.length > 0 ? (
                            run.issues.map((issue: any, i: number) => (
                                <div key={i} className="flex justify-between items-center py-0.5 gap-2 border-b border-slate-50 last:border-b-0">
                                   <span className="truncate">{issue.title || "Issue"}</span>
                                   <span className={`text-[8px] px-1 rounded uppercase font-bold shrink-0 ${issue.classification === 'missing' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}>
                                       {issue.classification || "N/A"}
                                   </span>
                                </div>
                            ))
                        ) : <span className="text-[10px] text-slate-400 italic">No bugs found</span>}
                      </div>
                    </button>
                  ))
                )}
      ...
      {isFullscreen && (
        <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col animate-fade-in text-white overflow-hidden select-none">
          ...
          <header className="bg-slate-950 border-b border-slate-800/80 p-3 px-6 flex flex-wrap items-center justify-between gap-4 shrink-0 shadow-xl relative z-30">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`px-3 py-1.5 bg-slate-800 hover:bg-slate-700 active:bg-slate-900 text-slate-200 font-bold rounded-lg text-xs transition-all flex items-center gap-1.5 ${isAuditing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} shadow-sm border border-slate-700`}
              >
                <RefreshCw size={13} className={isAuditing ? 'animate-spin' : ''} />
                <span>{isAuditing ? 'Syncing...' : 'Sync'}</span>
              </button>
          ...
          <div className="bg-slate-900/90 hidden backdrop-blur-md border-b border-slate-800/80 px-6 py-2.5 flex flex-wrap items-center justify-between gap-4 z-20 shrink-0 shadow-md">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
                <button
                  type="button"
                  onClick={() => setFullscreenCompareMode("side-by-side")}
                  className={`px-3.5 py-1.5 text-xs font-bold rounded-lg transition-all duration-200 flex items-center gap-1.5 cursor-pointer ${
                    fullscreenCompareMode === "side-by-side"
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                className={`px-3.5 py-1.8 text-xs font-bold rounded-xl border transition-all duration-200 cursor-pointer flex items-center gap-1.5 ${
                  showFullscreenAlignmentControls
          ...
          <div className="flex-1 overflow-hidden relative flex">
            ...
            {fullscreenCompareMode === "overlapped" ? (
              <div 
                className={`flex-1 h-full flex transition-all duration-500 ease-in-out relative bg-slate-950 overflow-auto p-8 select-none justify-center items-start ${
                  (selectedIssue || showFullscreenAlignmentControls) ? "w-1/2" : "w-full"
                }`}
              >
                {/* Fixed Overlay helper tag */}
                <div className="absolute top-4 left-4 bg-indigo-950/90 backdrop-blur-md px-3.5 py-2 text-xs font-bold text-slate-205 rounded-xl border border-indigo-900/60 z-30 flex items-center gap-2 shadow-lg">
                ...
                {showAlignmentPopup && (
                  <div className="absolute top-16 left-4 bg-slate-900 border border-slate-700 p-4 rounded-xl z-50 w-64 shadow-2xl space-y-4">
                     ...
                     <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-800">
                       <button
                         ...
                         className={`flex-1 text-[10px] font-bold py-1 px-2 rounded-md transition-all cursor-pointer text-center ${
                       ...
                       <button
                         ...
                         className={`flex-1 text-[10px] font-bold py-1 px-2 rounded-md transition-all cursor-pointer text-center ${
                           overlapAlignmentTab === "background"
                         ...
                         <div className="space-y-1">
                           <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => setOverlapYOffset(Math.max(-100, Math.min(100, overlapYOffset - 1)))}
                                className="w-6 h-6 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors cursor-pointer flex items-center justify-center border border-slate-700/50"
                                title="Decrease by 0.1"
                              >
                                <Minus size={10} strokeWidth={3} />
                         ...
                         <div className="space-y-1">
                           <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => setOverlapXOffset(Math.max(-100, Math.min(100, overlapXOffset - 1)))}
                                className="w-6 h-6 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors cursor-pointer flex items-center justify-center border border-slate-700/50"
                                title="Decrease by 0.1"
                              >
                                <Minus size={10} strokeWidth={3} />
                         ...
                         <div className="space-y-1">
                           <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => setBgOverlapYOffset(Math.max(-100, Math.min(100, bgOverlapYOffset - 1)))}
                                className="w-6 h-6 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors cursor-pointer flex items-center justify-center border border-slate-700/50"
                                title="Decrease by 0.1"
                              >
                                <Minus size={10} strokeWidth={3} />
                         ...
                         <div className="space-y-1">
                           <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                onClick={() => setBgOverlapXOffset(Math.max(-100, Math.min(100, bgOverlapXOffset - 1)))}
                                className="w-6 h-6 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors cursor-pointer flex items-center justify-center border border-slate-700/50"
                                title="Decrease by 0.1"
                              >
                                <Minus size={10} strokeWidth={3} />
                ...
                <div className="relative w-full max-w-[96%] h-[78vh] shrink-0 border border-slate-800 rounded-2xl bg-slate-950 flex items-start justify-center overflow-auto shadow-2xl dark-scroll p-6">
                    ...
                    {premiumIssues.map((issue, idx) => {
                      ...
                      return (
                        <div key={`over-issue-${issue.id}`}>
                          ...
                          >
                            <div 
                              className={`w-full h-full transition-all border-2 border-dashed flex items-center justify-center relative ${
                          ...
                          <button
                            ...
                            onMouseEnter={() => setIsHoveredIssueId(issue.id)}
                            onMouseLeave={() => setIsHoveredIssueId(null)}
                            className={`absolute w-8 h-8 rounded-full flex items-center justify-center text-xs font-mono font-bold cursor-pointer font-black transition-all z-35 border-2 border-white/90 shadow ${
                              isSelected || isHovered
              ...
              <div 
                className={`flex-1 h-full flex transition-all duration-500 ease-in-out ${
                  (selectedIssue || showFullscreenAlignmentControls) ? "w-1/2 translate-x-[-10%]" : "w-full"
                }`}
              >
                
                {/* FIGMA ORIGINAL SIDE */}
                <div className={`h-full border-r border-slate-800 flex flex-col relative overflow-hidden transition-all duration-500 ${
                  selectedIssue ? "w-0 opacity-0 bg-slate-950 pointer-events-none" : "w-1/2"
                }`}>
                  ...
                  <div className="flex-1 w-full h-full overflow-auto p-6 bg-slate-950 flex items-start justify-center">
                    <div 
                ...
                <div className={`h-full flex flex-col relative overflow-hidden transition-all duration-500 ${
                  selectedIssue ? "w-full bg-slate-950" : "w-1/2"
                }`}>
              ...
              <div className="absolute top-0 right-0 bottom-0 w-1/2 bg-slate-950 border-l border-slate-800 flex flex-col z-40 animate-slide-left select-text">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                          ...
                          >
                            <div className="truncate flex-1">
                              <div className="flex items-center gap-3">
                                ...
                                <span className={`text-[9px] uppercase font-bold tracking-wider px-1.5 rounded-sm ${
                                  issue.classification === "missing" ? "bg-rose-500/10 text-rose-400" :
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded text-[10px] uppercase tracking-widest font-black ${
                        selectedIssue.classification === "missing" ? "bg-rose-950/80 text-rose-300 border border-rose-900" :
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                        selectedIssue.severity === "critical" ? "bg-rose-600 text-white" :
                  ...
                  <div className="!hidden p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-4 animate-fade-in relative overflow-hidden">
                    ...
                    <div className="flex gap-2 pt-2 border-t border-slate-800">
                      <button
                        ...
                        className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 border-none transition-all cursor-pointer ${
          ...
          <div className="bg-slate-950/95 backdrop-blur-xl border border-slate-700 rounded-2xl p-5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] text-slate-200">
            <div className="flex items-center justify-between gap-4 mb-3">
              <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${
                selectedOverlayIssue.classification === "missing" ? "bg-rose-950/80 text-rose-300" :
              ...
              return (
                <div key={`print-issue-${issue.id}`} className="p-5 border border-slate-200 rounded-xl bg-slate-50/50 space-y-3" style={{ pageBreakInside: "avoid" }}>
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 text-[9px] font-mono tracking-widest uppercase font-bold rounded border ${
